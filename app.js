(() => {
  ...
  function karsaDisplayDivision(value) {
  ...
  function getIdentityId() {
    const u = state.user || {};
    if (u.employeeId) return String(u.employeeId).replace(/^KRSA-ITIT-/i, "KRSA-IT-");
    const division = karsaDisplayDivision(u.division);
  ...
  function drawLuxuryCardBackground(ctx, W, H, variant = "front") {
    const g = ctx.createLinearGradient(0, 0, W, H);
    g.addColorStop(0, "#080808");
    g.addColorStop(.45, variant === "back" ? "#191510" : "#15120d");
    g.addColorStop(1, "#050505");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, H);

    const glow = ctx.createRadialGradient(W * .84, H * .14, 8, W * .84, H * .14, W * .52);
    glow.addColorStop(0, "rgba(211,176,106,.20)");
    glow.addColorStop(.42, "rgba(201,164,92,.07)");
    ...
    ctx.save();
    ctx.globalAlpha = .055;
    ctx.strokeStyle = "#d3b06a";
    ctx.lineWidth = 1;
    for (let i = -H; i < W + H; i += Math.max(34, Math.round(W * .074))) {
      ctx.beginPath();
      ctx.moveTo(i, 0);
      ctx.lineTo(i + H, H);
      ctx.stroke();
    ...
    ctx.globalAlpha = .035;
    ctx.fillStyle = "#fff";
    const step = Math.max(18, Math.round(W * .035));
    for (let x = 18; x < W; x += step) for (let y = 18; y < H; y += step) ctx.fillRect(x, y, 1, 1);
    ctx.restore();

    ctx.strokeStyle = "rgba(211,176,106,.78)";
    ctx.lineWidth = Math.max(1.4, W / 430);
    roundRectCanvas(ctx, 2, 2, W - 4, H - 4, Math.round(W * .039));
    ctx.stroke();
    ctx.strokeStyle = "rgba(255,255,255,.075)";
    ctx.lineWidth = 1;
    roundRectCanvas(ctx, 9, 9, W - 18, H - 18, Math.round(W * .029));
    ctx.stroke();
  }

  async function drawIdentityFrontCanvas(ctx, W, H) {
    const baseW = 620, baseH = 391, sx = W / baseW, sy = H / baseH;
    const X = n => n * sx, Y = n => n * sy;
    drawLuxuryCardBackground(ctx, W, H, "front");

    const logo = await loadIdentityCanvasImage("assets/karsa-mark.jpg");
    if (logo) {
      roundRectCanvas(ctx, X(32), Y(28), X(30), Y(30), X(7));
      ctx.save(); ctx.clip(); ctx.drawImage(logo, X(32), Y(28), X(30), Y(30)); ctx.restore();
    }
    ctx.fillStyle = "#d3b06a";
    ctx.font = `900 ${Math.round(X(25))}px Arial`;
    ctx.fillText("KARSA", X(72), Y(50));
    ctx.fillStyle = "#bba77b";
    ctx.font = `700 ${Math.max(7, Math.round(X(8)))}px Arial`;
    ctx.textAlign = "right";
    ctx.fillText("EXECUTIVE ID", W - X(32), Y(43));
    ctx.textAlign = "left";

    const px = X(32), py = Y(112), pw = X(128), ph = Y(128);
    const avatar = await loadIdentityCanvasImage(state.user.avatarUrl || "");
    ctx.save();
    roundRectCanvas(ctx, px, py, pw, ph, X(18));
    ctx.clip();
    if (avatar) {
      // Match the web card's square, object-fit:cover photo treatment.
      const ar = avatar.naturalWidth / avatar.naturalHeight || 1;
      const target = pw / ph;
      let dw = pw, dh = ph, dx = px, dy = py;
      if (ar > target) { dw = ph * ar; dx = px - (dw - pw) / 2; }
      else { dh = pw / ar; dy = py - (dh - ph) / 2; }
      ctx.drawImage(avatar, dx, dy, dw, dh);
    } else {
      ctx.fillStyle = "#242424"; ctx.fillRect(px, py, pw, ph);
      ctx.fillStyle = "#d3b06a"; ctx.font = `800 ${Math.round(X(40))}px Arial`;
      ctx.textAlign = "center"; ctx.textBaseline = "middle";
      ctx.fillText(initials(state.user.name), px + pw / 2, py + ph / 2);
      ctx.textAlign = "left"; ctx.textBaseline = "alphabetic";
    }
    ctx.restore();
    ctx.strokeStyle = "rgba(201,164,92,.7)"; ctx.lineWidth = Math.max(1, X(1));
    roundRectCanvas(ctx, px, py, pw, ph, X(18)); ctx.stroke();

    const infoX = X(184);
    ctx.fillStyle = "#bba77b"; ctx.font = `800 ${Math.max(7, Math.round(X(8)))}px Arial`;
    ctx.fillText("AUTHORIZED MEMBER", infoX, Y(121));
    ctx.fillStyle = "#f5f2eb"; ctx.font = `800 ${Math.round(X(27))}px Arial`;
    drawCanvasWrapped(ctx, state.user.name || "—", infoX, Y(151), X(270), Y(28));
    ctx.fillStyle = "#d2bd91"; ctx.font = `600 ${Math.round(X(12))}px Arial`;
    drawCanvasWrapped(ctx, state.user.title || roleLabel(state.user.role) || "—", infoX, Y(178), X(270), Y(16));
    ctx.strokeStyle = "#806633"; ctx.lineWidth = Math.max(1, X(1));
    ctx.beginPath(); ctx.moveTo(infoX, Y(201)); ctx.lineTo(X(334), Y(201)); ctx.stroke();
    ctx.fillStyle = "#f0ede6"; ctx.font = `500 ${Math.round(X(9))}px Arial`;
    ctx.fillText(karsaDisplayDivision(state.user.division), infoX, Y(218));
    ctx.fillStyle = "#a99d86"; ctx.font = `500 ${Math.round(X(8))}px monospace`;
    ctx.fillText(`ID ${getIdentityId()}`, infoX, Y(234));

    const qrImg = document.querySelector("#identityQr img");
    const qx = X(488), qy = Y(112), qs = X(100);
    ctx.fillStyle = "#fff"; roundRectCanvas(ctx, qx, qy, qs, qs, X(12)); ctx.fill();
    if (qrImg && qrImg.complete && qrImg.naturalWidth) ctx.drawImage(qrImg, qx + X(8), qy + Y(8), X(84), Y(84));
    ctx.fillStyle = "#9e8b62"; ctx.font = `700 ${Math.max(6, Math.round(X(6)))}px Arial`; ctx.textAlign = "center";
    ctx.fillText("SCAN TO VERIFY", qx + qs / 2, Y(220));

    ctx.textAlign = "left"; ctx.fillStyle = "#9e8b62"; ctx.font = `700 ${Math.max(6, Math.round(X(7)))}px Arial`;
    ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", X(32), H - Y(22));
    ctx.fillStyle = "#d3b06a"; ctx.font = `800 ${Math.max(6, Math.round(X(7)))}px Arial`; ctx.textAlign = "right";
    ctx.fillText("ACTIVE", W - X(32), H - Y(22));
    ctx.textAlign = "left";
  }
  ...
  async function drawIdentityBackCanvas(ctx, W, H) {
    const baseW = 620, baseH = 391, sx = W / baseW, sy = H / baseH;
    const X = n => n * sx, Y = n => n * sy;
    drawLuxuryCardBackground(ctx, W, H, "back");

    const logo = await loadIdentityCanvasImage("assets/karsa-mark.jpg");
    if (logo) {
      roundRectCanvas(ctx, X(295), Y(34), X(30), Y(30), X(7));
      ctx.save(); ctx.clip(); ctx.drawImage(logo, X(295), Y(34), X(30), Y(30)); ctx.restore();
    }
    ctx.fillStyle = "#d3b06a"; ctx.font = `900 ${Math.round(X(25))}px Arial`; ctx.textAlign = "center";
    ctx.fillText("KARSA", W / 2, Y(90));
    ctx.fillStyle = "#c9ad72"; ctx.font = `700 ${Math.max(7, Math.round(X(8)))}px Arial`;
    ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", W / 2, Y(108));

    const items = [
      ["COMPANY", "PT Karsa Lifestyle Nusantara"],
      ["IDENTITY", "Official Internal Employee Card"],
      ["VERIFICATION", "Scan QR on front"],
      ["STATUS", "ACTIVE / AUTHORIZED"]
    ];
    const boxW = X(255), boxH = Y(70), gap = X(10), x1 = W / 2 - boxW - gap / 2, x2 = W / 2 + gap / 2;
    const ys = [Y(132), Y(211)];
    items.forEach((it, i) => {
      const x = i % 2 ? x2 : x1, y = i < 2 ? ys[0] : ys[1];
      ctx.fillStyle = "rgba(255,255,255,.035)"; ctx.strokeStyle = "rgba(201,164,92,.25)"; ctx.lineWidth = 1;
      roundRectCanvas(ctx, x, y, boxW, boxH, X(10)); ctx.fill(); ctx.stroke();
      ctx.fillStyle = "#8f8169"; ctx.font = `700 ${Math.max(5, Math.round(X(5)))}px Arial`; ctx.textAlign = "left"; ctx.fillText(it[0], x + X(12), y + Y(18));
      ctx.fillStyle = "#eee9df"; ctx.font = `700 ${Math.max(7, Math.round(X(8)))}px Arial`; drawCanvasWrapped(ctx, it[1], x + X(12), y + Y(37), boxW - X(24), Y(10));
    });

    ctx.strokeStyle = "rgba(211,176,106,.30)"; ctx.beginPath(); ctx.moveTo(X(150), Y(300)); ctx.lineTo(W - X(150), Y(300)); ctx.stroke();
    ctx.fillStyle = "#c9ad72"; ctx.font = `700 ${Math.max(6, Math.round(X(7)))}px Arial`; ctx.textAlign = "center";
    ctx.fillText(`KARSA IDENTITY • ${getIdentityId()}`, W / 2, Y(323));
    ctx.fillStyle = "#77736b"; ctx.font = `500 ${Math.max(6, Math.round(X(6)))}px Arial`;
    ctx.fillText("This card identifies an authorized member of PT Karsa Lifestyle Nusantara.", W / 2, Y(342));
    ctx.textAlign = "left";
  }

  async function downloadIdentityCard(side) {
    if (!state.user) return;
    await renderIdentityQr();
    // Compact output, same 1.586 ratio and proportions as the web card.
    const W = 720, H = Math.round(W / 1.586);
    ...
    return new Promise(resolve => {
      ...
      img.onerror = async () => {
        try {
          ...
          fallback.onload = () => { URL.revokeObjectURL(objectUrl); resolve(fallback); };
          fallback.onerror = () => { URL.revokeObjectURL(objectUrl); resolve(null); };
...
function openProfile() {
  ...
  function karsaEmployeeId(p) {
    if (p?.employee_id) return String(p.employee_id).replace(/^KRSA-ITIT-/i, "KRSA-IT-");
    const division = karsaDisplayDivision(p?.division);
    const code = ({Finance:"FIN",IT:"IT",Operasional:"OPS","R&D":"RND"})[division] || "KRS";
  ...
  function karsaAddNav() {
    ...
    wrap.addEventListener("click", e => {
      ...
      if (b) {
        const type = b.dataset.karsaExt;
        closeKarsaSidebar();
        b.classList.add("is-opening");
        window.clearTimeout(window.__karsaToolOpenTimer);
        window.__karsaToolOpenTimer = window.setTimeout(() => {
          b.classList.remove("is-opening");
          openKarsaExtension(type);
        }, 120);
      }
    });
  }
  ...
  async function handleKarsaVerifyQuery(){
    const rawId = new URLSearchParams(location.search).get("verify");
    if (!rawId) return;
    const inputId = String(rawId).trim();
    const normalizedId = inputId.replace(/^KRSA-ITIT-/i, "KRSA-IT-");
    karsaVerifyModal();
    $("karsaVerifyModal").classList.remove("hidden");
    $("karsaVerifyBody").innerHTML = `<div class="karsa-verify-loading"><span class="karsa-verify-spinner"></span><div><strong>Memverifikasi identitas</strong><small>${esc(normalizedId)}</small></div></div>`;
    if (!client) {
      $("karsaVerifyBody").innerHTML = `<div class="karsa-verify-result error"><strong>Verification unavailable</strong><span>Sistem belum terhubung.</span></div>`;
      return;
    }
    const {data,error} = await client.rpc("verify_employee", {p_employee_id: normalizedId});
    const p = Array.isArray(data) ? data[0] : data;
    if (error || !p) {
      $("karsaVerifyBody").innerHTML = `<div class="karsa-verify-result error"><div class="karsa-verify-status">NOT VERIFIED</div><strong>ID tidak ditemukan</strong><span>ID <b>${esc(normalizedId)}</b> tidak ditemukan atau tidak aktif.</span></div>`;
      return;
    }
    const division = karsaDisplayDivision(p.division);
    const employeeId = karsaEmployeeId(p);
    const photo = p.avatar_url || "";
    const photoHtml = photo
      ? `<img src="${esc(photo)}" alt="Foto ${esc(p.name || "karyawan")}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`
      : "";
    $("karsaVerifyBody").innerHTML = `
      <div class="karsa-verify-card">
        <div class="karsa-verify-card-top">
          <div class="karsa-verify-photo">${photoHtml}<span style="display:${photo ? "none" : "flex"}">${esc(initials(p.name))}</span></div>
          <div class="karsa-verify-main">
            <div class="karsa-verify-status">✓ VERIFIED</div>
            <h3>${esc(p.name || "—")}</h3>
            <p>${esc(p.title || roleLabel(p.role) || "—")}</p>
          </div>
        </div>
        <div class="karsa-verify-details">
          <div><small>DIVISI</small><b>${esc(division)}</b></div>
          <div><small>EMPLOYEE ID</small><b>${esc(employeeId)}</b></div>
          <div><small>STATUS</small><b>ACTIVE / AUTHORIZED</b></div>
        </div>
        <div class="karsa-verify-footer">Data identitas ditemukan pada sistem KARSA.</div>
      </div>`;
  }

  /* Remove browser tap/keyboard flash and make interactive KARSA controls feel integrated. */
  function karsaElegantInteractionStyle(){
    if($('karsaElegantInteractionStyle'))return;
    const s=document.createElement('style');s.id='karsaElegantInteractionStyle';s.textContent=`
      button,a,label,input,select,textarea,.nav-item,.karsa-person-avatar-btn,.karsa-profile-avatar,.mobile-menu-btn{ -webkit-tap-highlight-color:transparent!important; }
      button:focus,a:focus,label:focus,input:focus,select:focus,textarea:focus{outline:none;}
      button::-moz-focus-inner{border:0;}
      .karsa-ext-nav button,.karsa-profile-btn,.karsa-profile-close,.karsa-id-controls button,.karsa-mini-btn,.nav-item,.mobile-menu-btn,.karsa-person-avatar-btn{transition:background-color .18s ease,border-color .18s ease,color .18s ease,box-shadow .18s ease,transform .18s ease;position:relative;}
      .karsa-ext-nav button:hover,.karsa-profile-btn:hover,.karsa-id-controls button:hover,.karsa-mini-btn:hover{box-shadow:0 8px 24px rgba(0,0,0,.08);}
      ...
      .karsa-ext-nav button:focus-visible,.karsa-profile-btn:focus-visible,.karsa-id-controls button:focus-visible,.karsa-mini-btn:focus-visible{box-shadow:0 0 0 2px rgba(201,164,92,.28)!important;}
      .karsa-profile-avatar{box-shadow:0 10px 28px rgba(0,0,0,.14),0 0 0 1px rgba(201,164,92,.15)!important;}
      .karsa-profile-avatar:active,.karsa-person-avatar-btn:active{transform:scale(.985);}
      .karsa-ext-nav button:hover{background:rgba(201,164,92,.08)!important;color:#d1b06e!important;}
      .karsa-ext-nav button:active{background:rgba(201,164,92,.13)!important;}
      .karsa-ext-nav button.is-opening{background:rgba(201,164,92,.10)!important;color:#d1b06e!important;transform:translateY(1px);box-shadow:0 5px 18px rgba(0,0,0,.06)!important;}
      .karsa-profile-close:hover{background:#f0eadf!important;color:#7a5b29!important;}
      .karsa-id-controls button{box-shadow:none!important;}
      .karsa-id-controls button:hover{border-color:#c9a45c!important;color:#755b2d!important;}
      .karsa-photo-viewer-close{ -webkit-tap-highlight-color:transparent!important; outline:none!important;}
      .karsa-verify-loading{display:flex;align-items:center;gap:12px;padding:20px;border:1px solid #e5dfd2;border-radius:18px;background:#fff;}
      .karsa-verify-loading strong{display:block;color:#171717;font-size:14px}.karsa-verify-loading small{display:block;color:#8a8174;margin-top:4px;font-size:11px;}
      .karsa-verify-spinner{width:18px;height:18px;border:2px solid #e5dfd2;border-top-color:#c9a45c;border-radius:50%;animation:karsaVerifySpin .7s linear infinite;flex:0 0 auto;}
      @keyframes karsaVerifySpin{to{transform:rotate(360deg)}}
      .karsa-verify-card{background:linear-gradient(145deg,#fffdf9,#f5f1e8);border:1px solid rgba(201,164,92,.34);border-radius:20px;padding:18px;box-shadow:0 18px 45px rgba(30,24,15,.08);}
      .karsa-verify-card-top{display:flex;gap:14px;align-items:center;}.karsa-verify-main{min-width:0;}.karsa-verify-main h3{margin:4px 0 3px;color:#161616;font-size:20px;line-height:1.15}.karsa-verify-main p{margin:0;color:#776c5c;font-size:12px;}
      .karsa-verify-status{display:inline-flex;align-items:center;gap:5px;color:#7a5d27;font-size:9px;font-weight:900;letter-spacing:.15em;}
      .karsa-verify-details{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:16px;}.karsa-verify-details>div{padding:10px 11px;border:1px solid #e4dccd;border-radius:12px;background:rgba(255,255,255,.65);min-width:0}.karsa-verify-details small{display:block;color:#9a8e7c;font-size:7px;letter-spacing:.14em;font-weight:800;margin-bottom:4px}.karsa-verify-details b{display:block;color:#26221d;font-size:10px;line-height:1.25;word-break:break-word}.karsa-verify-footer{margin-top:13px;color:#8b8276;font-size:10px;}
      .karsa-verify-result{padding:20px;border:1px solid #e5dfd2;border-radius:18px;background:#fff;display:flex;flex-direction:column;gap:7px}.karsa-verify-result strong{color:#222}.karsa-verify-result span{color:#777;font-size:11px}.karsa-verify-result.error .karsa-verify-status{color:#8a5b35;}
      @media(max-width:600px){.karsa-verify-details{grid-template-columns:1fr 1fr}.karsa-verify-main h3{font-size:17px}.karsa-verify-card{padding:14px;}}
   ...
   * BOOT / STARTUP
   * =========================================================
   * V10: restore the application boot sequence. The previous
   * V9 build lost the boot() invocation, leaving the loading
  ...
  async function boot() {
    try {
      ...
      /* Always release the loading screen. Do not wait for
         Supabase/network requests before showing the login. */
      const releaseBoot = () => {
        const bootEl = $("boot");
        const loginEl = $("loginPage");
        if (bootEl) bootEl.classList.add("out");
        if (loginEl && !state.user) loginEl.classList.remove("hidden");
        if (CFG.DEMO_MODE) $("demoBox")?.classList.remove("hidden");
      };

      window.setTimeout(() => {
        $("bootStatus") && ($("bootStatus").textContent = "Opening secure workspace");
      }, 500);

      window.setTimeout(releaseBoot, 1250);
      ...
      if (client) {
        try {
          ...
          console.warn("KARSA session restore skipped:", sessionErr);
          releaseBoot();
        }
        ...
        client.auth.onAuthStateChange(async (event, session) => {
          ...
          if (session && !state.user) {
            try {
              ...
              console.warn("KARSA auth state restore skipped:", authErr);
              releaseBoot();
            }
          }
        });
      ...
      /* Public verification URL, e.g. ?verify=KRSA-IT-001 */
      window.setTimeout(() => {
        try { handleKarsaVerifyQuery(); }
        catch (verifyErr) { console.warn("KARSA verification skipped:", verifyErr); }
      }, 150);
    } catch (bootErr) {
      console.error("KARSA boot error:", bootErr);
