(() => {
  ...
  async function drawIdentityFrontCanvas(ctx, W, H) {
    ...
    ctx.textAlign = "right";
    ctx.fillText("IDENTITY • 01", W - pad, 106);
    ctx.fillStyle = "#857455";
    ctx.font = "700 26px Arial";
    ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", W - pad, 140);
    ctx.textAlign = "left";
    ...
    drawCanvasWrapped(ctx, state.user.name || "—", infoX, 306, 710, 76);

    ctx.fillStyle = "#dfc58f";
    ctx.font = "700 43px Arial";
    drawCanvasWrapped(ctx, state.user.title || roleLabel(state.user.role) || "—", infoX, 384, 650, 48);

    ctx.strokeStyle = "rgba(211,176,106,.65)";
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(infoX, 432);
    ctx.lineTo(infoX + 285, 432);
    ctx.stroke();

    ctx.fillStyle = "#f4efe6";
    ctx.font = "800 42px Arial";
    ctx.fillText(`DIVISION  ${normalizeKarsaDivision(state.user.division).toUpperCase()}`, infoX, 486);

    ctx.fillStyle = "#b7a98d";
    ctx.font = "700 36px monospace";
    ctx.fillText(`ID  ${getIdentityId()}`, infoX, 532);

    // Compact status badge under identity information.
    ctx.fillStyle = "rgba(211,176,106,.10)";
    ctx.strokeStyle = "rgba(211,176,106,.38)";
    ctx.lineWidth = 2;
    canvasRoundRect(ctx, infoX, 558, 220, 60, 17);
    ctx.fill(); ctx.stroke();
    ctx.fillStyle = "#dfbd70";
    ctx.font = "900 28px Arial";
    ctx.fillText("●  ACTIVE", infoX + 25, 598);

    // Larger QR, balanced against the photo instead of leaving a large dead zone.
    const qrSize = 310, qrX = W - pad - qrSize, qrY = 182;
    ctx.fillStyle = "#fff";
    ...
    if (qrImg && qrImg.complete) {
      ...
      drawCanvasFit(ctx, qrImg, qrX + 18, qrY + 18, qrSize - 36, qrSize - 36);
      ctx.imageSmoothingEnabled = true;
    }
    ctx.fillStyle = "#b69b69";
    ctx.font = "900 29px Arial";
    ctx.fillText("SCAN TO VERIFY", qrX, qrY + qrSize + 42);

    // Refined footer band: larger type, clear spacing, and separated from both guide lines.
    const stripY = 728, footerBottomY = H - 58;
    ctx.strokeStyle = "rgba(211,176,106,.48)";
    ctx.lineWidth = 2.5;
    ctx.beginPath(); ctx.moveTo(pad, stripY); ctx.lineTo(W - pad, stripY); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(pad, footerBottomY); ctx.lineTo(W - pad, footerBottomY); ctx.stroke();

    ctx.fillStyle = "#9b8660";
    ctx.font = "800 34px Arial";
    ctx.fillText("OFFICIAL EMPLOYEE IDENTITY", pad, stripY + 50);
    ctx.fillStyle = "#dfbd70";
    ctx.font = "900 34px Arial";
    ctx.fillText(`KARSA ID  •  ${getIdentityId()}`, pad, stripY + 94);

    ctx.fillStyle = "#9b8660";
    ctx.font = "800 34px Arial";
    ctx.textAlign = "right";
    ctx.fillText("AUTHORIZED • INTERNAL USE", W - pad, stripY + 50);
    ctx.fillStyle = "#dfbd70";
    ctx.font = "900 34px Arial";
    ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", W - pad, stripY + 94);
    ctx.textAlign = "left";
  }
  ...
  async function drawIdentityBackCanvas(ctx, W, H) {
    ...
    // Premium rear layout: stronger typography, fewer micro-labels, and balanced panels.
    if (logo) {
      ctx.save();
      canvasRoundRect(ctx, pad, 54, 96, 96, 22); ctx.clip();
      drawCanvasCover(ctx, logo, pad, 54, 96, 96);
      ctx.restore();
      ctx.strokeStyle = "rgba(211,176,106,.84)"; ctx.lineWidth = 3;
      canvasRoundRect(ctx, pad, 54, 96, 96, 22); ctx.stroke();
    }

    ctx.fillStyle = "#dfbd70"; ctx.font = "900 78px Arial"; ctx.fillText("KARSA", 190, 125);
    ctx.fillStyle = "#c3aa78"; ctx.font = "800 35px Arial"; ctx.fillText("OFFICIAL EMPLOYEE IDENTITY", 190, 163);

    ctx.fillStyle = "#faf7f0"; ctx.font = "900 49px Arial"; ctx.fillText("PT Karsa Lifestyle Nusantara", pad, 235);
    ctx.fillStyle = "#b9aa91"; ctx.font = "700 31px Arial"; ctx.fillText("KARSA INTERNAL IDENTITY SYSTEM", pad, 274);

    // Elegant oversized branding in the lower negative space.
    ctx.save();
    ctx.globalAlpha = 0.065;
    ctx.fillStyle = "#d8b76f";
    ctx.font = "900 205px Arial";
    ctx.textAlign = "center";
    ctx.fillText("KARSA", W * 0.48, 790);
    ctx.globalAlpha = 0.035;
    ctx.strokeStyle = "#d8b76f";
    ctx.lineWidth = 3;
    ctx.strokeText("KARSA", W * 0.48, 790);
    ctx.restore();
    ctx.textAlign = "left";

    // Larger luxury information cards. Labels remain readable but the value is the visual focus.
    const boxW = 455, boxH = 124, gapX = 22, gapY = 22;
    const x1 = pad, x2 = pad + boxW + gapX, y1 = 318, y2 = y1 + boxH + gapY;
    const items = [
    ...
    items.forEach((it, i) => {
      ...
      ctx.strokeStyle = "rgba(211,176,106,.44)";
      ctx.lineWidth = 2.5;
      canvasRoundRect(ctx, x, y, boxW, boxH, 22); ctx.fill(); ctx.stroke();

      // Small gold accent bar adds hierarchy without making the panel busy.
      ctx.fillStyle = "#d3b06a";
      canvasRoundRect(ctx, x + 22, y + 22, 6, boxH - 44, 3); ctx.fill();

      ctx.fillStyle = "#c8ad78"; ctx.font = "900 31px Arial";
      ctx.fillText(it[0], x + 46, y + 38);
      ctx.fillStyle = "#faf7f0"; ctx.font = "800 33px Arial";
      drawCanvasWrapped(ctx, it[1], x + 46, y + 82, boxW - 88, 36);
    });

    // Company information QR — intentionally large and clean for real scanning.
    const companyQrText = [
      "PT KARSA LIFESTYLE NUSANTARA",
      "Official Company Information",
      "KARSA Internal Identity System",
      "Bentuk Usaha: Perseroan Perorangan",
    ...
    try {
      const qr = await loadQrLibrary();
      const q = qr(0, "Q");
      q.addData(companyQrText);
      q.make();
      companyQr = await loadIdentityCanvasImage(q.createDataURL(6, 0));
    } catch (_) {}

    const qrSize = 315, qrX = W - pad - qrSize, qrY = 302;
    ctx.fillStyle = "rgba(255,255,255,.96)";
    canvasRoundRect(ctx, qrX - 16, qrY - 16, qrSize + 32, qrSize + 32, 28); ctx.fill();
    ctx.strokeStyle = "rgba(211,176,106,.62)"; ctx.lineWidth = 2;
    canvasRoundRect(ctx, qrX - 16, qrY - 16, qrSize + 32, qrSize + 32, 28); ctx.stroke();
    if (companyQr) {
      ctx.imageSmoothingEnabled = false;
      drawCanvasFit(ctx, companyQr, qrX, qrY, qrSize, qrSize);
      ctx.imageSmoothingEnabled = true;
    }
    ctx.fillStyle = "#dfbd70"; ctx.font = "900 34px Arial";
    ctx.textAlign = "center"; ctx.fillText("COMPANY INFORMATION", qrX + qrSize / 2, qrY + qrSize + 52);
    ctx.fillStyle = "#b9aa91"; ctx.font = "800 29px Arial";
    ctx.fillText("SCAN FOR COMPANY INFO", qrX + qrSize / 2, qrY + qrSize + 90);

    // Footer typography is intentionally larger so it remains readable without zooming.
    ctx.strokeStyle = "rgba(211,176,106,.42)"; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(pad, H - 128); ctx.lineTo(W - pad, H - 128); ctx.stroke();
    ctx.fillStyle = "#dfbd70"; ctx.font = "900 39px Arial";
    ctx.textAlign = "center"; ctx.fillText(`KARSA IDENTITY  •  ${getIdentityId()}`, W / 2, H - 82);
    ctx.fillStyle = "#aaa08f"; ctx.font = "700 31px Arial";
    ctx.fillText("AUTHORIZED INTERNAL USE • PT KARSA LIFESTYLE NUSANTARA", W / 2, H - 42);
    ctx.textAlign = "left";
  }
