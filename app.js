(() => {
    ...
    identityModal.innerHTML = `
      <div class="karsa-identity-shell" role="dialog" aria-modal="true" aria-label="Kartu Identitas KARSA">
        ...
        <div class="karsa-id-controls">
          ...
          <button id="printIdentityBtn" type="button">Cetak</button><button id="downloadIdentityBtn" type="button">Download Card</button>
        </div>
  ...
  async function downloadIdentityCard() {
    if (!state.user) return;
    const card = $("karsaIdentityCard");
    const face = card?.querySelector(".karsa-id-front");
    if (!face) return;
    const w = 1200, h = 757;
    const canvas = document.createElement("canvas"); canvas.width=w; canvas.height=h;
    const ctx=canvas.getContext("2d");
    const bg=ctx.createLinearGradient(0,0,w,h); bg.addColorStop(0,"#080808"); bg.addColorStop(.55,"#17130d"); bg.addColorStop(1,"#080808"); ctx.fillStyle=bg; ctx.fillRect(0,0,w,h);
    ctx.strokeStyle="#9b7b3f"; ctx.lineWidth=3; ctx.strokeRect(3,3,w-6,h-6);
    // KARSA brand mark: restrained gold monogram + wordmark.
    ctx.fillStyle="#d3b06a"; ctx.font="900 58px Arial"; ctx.letterSpacing="8px"; ctx.fillText("KARSA",70,90);
    ctx.fillStyle="#bba77b"; ctx.font="700 18px Arial"; ctx.fillText("EXECUTIVE ID",930,88);
    const px=70, py=180, ps=240; ctx.fillStyle="#242424"; roundRect(ctx,px,py,ps,ps,28); ctx.fill();
    if (state.user.avatarUrl) {
      try { const im=new Image(); im.crossOrigin="anonymous"; im.src=state.user.avatarUrl; await new Promise((res,rej)=>{im.onload=res;im.onerror=rej}); ctx.save(); roundRect(ctx,px,py,ps,ps,28); ctx.clip(); ctx.drawImage(im,px,py,ps,ps); ctx.restore(); } catch(_) { drawInitials(ctx,px,py,ps); }
    } else drawInitials(ctx,px,py,ps);
    ctx.fillStyle="#fff"; ctx.font="800 48px Arial"; wrapCanvasText(ctx,state.user.name||"—",370,245,730,58);
    ctx.fillStyle="#d2bd91"; ctx.font="600 24px Arial"; wrapCanvasText(ctx,state.user.title||roleLabel(state.user.role)||"—",370,325,730,34);
    ctx.strokeStyle="#806633"; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(370,365); ctx.lineTo(620,365); ctx.stroke();
    ctx.fillStyle="#eee"; ctx.font="500 24px Arial"; ctx.fillText(state.user.division||"—",370,420);
    ctx.fillStyle="#a99d86"; ctx.font="500 20px monospace"; ctx.fillText(`ID ${getIdentityId()}`,370,458);
    ctx.fillStyle="#9e8b62"; ctx.font="700 15px Arial"; ctx.fillText("PT KARSA LIFESTYLE NUSANTARA",70,705);
    ctx.fillStyle="#d3b06a"; ctx.font="800 16px Arial"; ctx.textAlign="right"; ctx.fillText("ACTIVE",1130,705); ctx.textAlign="left";
    canvas.toBlob(blob=>{ if(!blob){showToast("Kartu gagal dibuat.");return;} const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`KARSA-ID-${getIdentityId()}.png`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);showToast("Kartu identitas berhasil diunduh."); },"image/png");

    function drawInitials(c,x,y,size){c.fillStyle="#242424";c.fillRect(x,y,size,size);c.fillStyle="#d3b06a";c.font=`800 ${Math.round(size*.28)}px Arial`;c.textAlign="center";c.textBaseline="middle";c.fillText(initials(state.user.name),x+size/2,y+size/2);c.textAlign="left";c.textBaseline="alphabetic";}
    function wrapCanvasText(c,text,x,y,maxWidth,lineHeight){const words=String(text).split(/\s+/);let line="";for(const word of words){const test=line?line+" "+word:word;if(c.measureText(test).width>maxWidth&&line){c.fillText(line,x,y);line=word;y+=lineHeight;}else line=test;}c.fillText(line,x,y);}
