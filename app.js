(() => {
  ...
  async function drawIdentityFrontCanvas(ctx, W, H) {
    ...
    const qrSize = 290, qrX = W - pad - qrSize, qrY = 190;
    ctx.fillStyle = "#fff";
    ...
    ctx.fillText("SCAN TO VERIFY", qrX, qrY + qrSize + 40);
    ...
    ctx.fillText("OFFICIAL EMPLOYEE IDENTITY", pad, stripY + 45);
    ctx.fillStyle = "#d3b06a";
    ctx.font = "900 30px Arial";
    ctx.fillText(`KARSA ID  •  ${getIdentityId()}`, pad, stripY + 82);

    ctx.fillStyle = "#877654";
    ctx.font = "800 30px Arial";
    ctx.textAlign = "right";
    ctx.fillText("AUTHORIZED • INTERNAL USE", W - pad, stripY + 45);
    ctx.fillStyle = "#dfbd70";
    ctx.font = "900 30px Arial";
    ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", W - pad, stripY + 82);
    ctx.textAlign = "left";
  }
  ...
  async function drawIdentityBackCanvas(ctx, W, H) {
    ...
    items.forEach((it, i) => {
      ...
      ctx.fillStyle = "#c8ad78"; ctx.font = "900 29px Arial";
      ctx.fillText(it[0], x + 46, y + 38);
      ctx.fillStyle = "#faf7f0"; ctx.font = "800 35px Arial";
      drawCanvasWrapped(ctx, it[1], x + 46, y + 84, boxW - 70, 38);
    });
    ...
    const qrSize = 290, qrX = W - pad - qrSize, qrY = 314;
    ctx.fillStyle = "rgba(255,255,255,.96)";
    canvasRoundRect(ctx, qrX - 18, qrY - 18, qrSize + 36, qrSize + 36, 30); ctx.fill();
    ctx.strokeStyle = "rgba(211,176,106,.62)"; ctx.lineWidth = 2;
    canvasRoundRect(ctx, qrX - 18, qrY - 18, qrSize + 36, qrSize + 36, 30); ctx.stroke();
    if (companyQr) {
      ctx.imageSmoothingEnabled = false;
      drawCanvasFit(ctx, companyQr, qrX, qrY, qrSize, qrSize);
      ctx.imageSmoothingEnabled = true;
    }
    ctx.fillStyle = "#dfbd70"; ctx.font = "900 31px Arial";
    ctx.textAlign = "center"; ctx.fillText("COMPANY INFORMATION", qrX + qrSize / 2, qrY + qrSize + 50);
    ctx.fillStyle = "#b9aa91"; ctx.font = "800 25px Arial";
    ctx.fillText("SCAN FOR COMPANY INFO", qrX + qrSize / 2, qrY + qrSize + 84);

    // Footer typography is intentionally larger so it remains readable without zooming.
    ctx.fillStyle = "#dfbd70"; ctx.font = "900 35px Arial";
    ctx.textAlign = "center"; ctx.fillText(`KARSA IDENTITY  •  ${getIdentityId()}`, W / 2, H - 91);
    ctx.fillStyle = "#aaa08f"; ctx.font = "700 28px Arial";
    ctx.fillText("Valid for authorized internal use of PT Karsa Lifestyle Nusantara.", W / 2, H - 53);
    ctx.textAlign = "left";
  }
