(() => {
  ...
  function renderPeople(people){
    ...
    grid.innerHTML=people.length?people.map(p=>{const id=karsaEmployeeId(p);const photo=p.avatar_url?`url("${p.avatar_url}")`:"";return `<article class="karsa-person"><div class="karsa-person-avatar" style="background-image:${photo}">${p.avatar_url?"":esc(initials(p.name))}</div><div><strong>${esc(p.name||"—")}</strong><small>${esc(p.title||roleLabel(p.role)||"—")}</small><small>${esc(p.division||"—")}</small><small class="karsa-person-id">${esc(id)}</small></div></article>`}).join(""):"<div class=\"karsa-empty-mini\">Tidak ada data.</div>";
  }
