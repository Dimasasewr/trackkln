(() => {
  ...
  async function downloadIdentityCard(side) {
    ...
    gradient.addColorStop(.55, "#17130d");
    gradient.addColorStop(1, "#080808");
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, W, H);
    ctx.strokeStyle = "#9b7835";
    ctx.lineWidth = 3;
    ctx.strokeRect(2, 2, W - 4, H - 4);

    if (side === "back") {
      await drawIdentityBackCanvas(ctx, W, H);
    } else {
      await drawIdentityFrontCanvas(ctx, W, H);
    }
  ...
  async function drawIdentityFrontCanvas(ctx, W, H) {
    ...
    if (logo) {
      roundRectCanvas(ctx, 46, 42, 52, 52, 11);
      ctx.save(); ctx.clip(); ctx.drawImage(logo, 46, 42, 52, 52); ctx.restore();
    }
    ctx.fillStyle = "#d3b06a"; ctx.font = "900 44px Arial"; ctx.fillText("KARSA", 114, 82);
    ctx.fillStyle = "#bba77b"; ctx.font = "700 12px Arial"; ctx.fillText("EXECUTIVE ID", W - 170, 76);

    const px = 46, py = 142, pw = 166, ph = 206;
    const avatar = await loadIdentityCanvasImage(state.user.avatarUrl || "");
    ctx.save(); roundRectCanvas(ctx, px, py, pw, ph, 20); ctx.clip();
    if (avatar) ctx.drawImage(avatar, px, py, pw, ph);
    else { ctx.fillStyle = "#242424"; ctx.fillRect(px, py, pw, ph); ctx.fillStyle = "#d3b06a"; ctx.font = "800 54px Arial"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(initials(state.user.name), px + pw/2, py + ph/2); ctx.textAlign = "left"; ctx.textBaseline = "alphabetic"; }
    ctx.restore();
    ctx.strokeStyle = "#806633"; ctx.lineWidth = 2; roundRectCanvas(ctx, px, py, pw, ph, 20); ctx.stroke();

    ctx.fillStyle = "#bba77b"; ctx.font = "700 11px Arial"; ctx.fillText("AUTHORIZED MEMBER", 250, 155);
    ctx.fillStyle = "#f5f2eb"; ctx.font = "800 34px Arial"; drawCanvasWrapped(ctx, state.user.name || "—", 250, 205, 500, 38);
    ctx.fillStyle = "#d2bd91"; ctx.font = "600 17px Arial"; drawCanvasWrapped(ctx, state.user.title || roleLabel(state.user.role) || "—", 250, 270, 440, 23);
    ctx.strokeStyle = "#806633"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(250, 300); ctx.lineTo(440, 300); ctx.stroke();
    ctx.fillStyle = "#f0ede6"; ctx.font = "500 15px Arial"; ctx.fillText(state.user.division || "—", 250, 330);
    ctx.fillStyle = "#a99d86"; ctx.font = "500 13px monospace"; ctx.fillText(`ID ${getIdentityId()}`, 250, 356);

    const qrImg = document.querySelector("#identityQr img");
    ctx.fillStyle = "#fff"; roundRectCanvas(ctx, W - 148, 156, 100, 100, 14); ctx.fill();
    if (qrImg && qrImg.complete) ctx.drawImage(qrImg, W - 138, 166, 80, 80);
    ctx.fillStyle = "#8f8067"; ctx.font = "700 9px Arial"; ctx.fillText("SCAN TO VERIFY", W - 148, 276);

    ctx.fillStyle = "#9e8b62"; ctx.font = "700 11px Arial"; ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", 46, H - 28);
    ctx.fillStyle = "#d3b06a"; ctx.font = "800 11px Arial"; ctx.textAlign = "right"; ctx.fillText("ACTIVE", W - 46, H - 28); ctx.textAlign = "left";
  }

  async function drawIdentityBackCanvas(ctx, W, H) {
    const logo = await loadIdentityCanvasImage("assets/karsa-mark.jpg");
    if (logo) {
      roundRectCanvas(ctx, W/2 - 26, 56, 52, 52, 11);
      ctx.save(); ctx.clip(); ctx.drawImage(logo, W/2 - 26, 56, 52, 52); ctx.restore();
    }
    ctx.fillStyle = "#d3b06a"; ctx.font = "900 40px Arial"; ctx.textAlign = "center"; ctx.fillText("KARSA", W/2, 148);
    ctx.fillStyle = "#c9ad72"; ctx.font = "700 12px Arial"; ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", W/2, 176);
    ...
    const boxW = 350, boxH = 72, gap = 14, x1 = W/2 - boxW - gap/2, x2 = W/2 + gap/2, y1 = 214, y2 = 300;
    items.forEach((it, i) => {
      const x = i % 2 ? x2 : x1, y = i < 2 ? y1 : y2;
      ctx.fillStyle = "rgba(255,255,255,.035)"; ctx.strokeStyle = "rgba(201,164,92,.28)"; ctx.lineWidth = 1;
      roundRectCanvas(ctx, x, y, boxW, boxH, 12); ctx.fill(); ctx.stroke();
      ctx.fillStyle = "#8f8169"; ctx.font = "700 8px Arial"; ctx.textAlign = "left"; ctx.fillText(it[0], x + 16, y + 22);
      ctx.fillStyle = "#eee9df"; ctx.font = "700 13px Arial"; drawCanvasWrapped(ctx, it[1], x + 16, y + 46, boxW - 32, 15);
    });
    ctx.fillStyle = "#c9ad72"; ctx.font = "700 9px Arial"; ctx.textAlign = "center"; ctx.fillText(`KARSA IDENTITY • ${getIdentityId()}`, W/2, H - 38);
    ctx.fillStyle = "#77736b"; ctx.font = "9px Arial"; ctx.fillText("This card identifies an authorized member of PT Karsa Lifestyle Nusantara.", W/2, H - 20);
    ctx.textAlign = "left";
  }
...
function openProfile() {
  ...
  function initKarsaExtensions(){
    karsaExtStyles();karsaAddNav();karsaMakeOverlay();bindJobModalTouchScroll();
  ...
  /* =========================================================
   * V8 POLISH / IDENTITY DOWNLOAD + CLICK UX + VERIFY FIX
   * ========================================================= */
  ...
  async function karsaImageForCanvas(url) {
    if (!url) return null;
    try {
    ...
    return await loadIdentityCanvasImage(url);
  }

  function drawLuxuryBackground(ctx, W, H, variant='front') {
    const g = ctx.createLinearGradient(0,0,W,H);
    g.addColorStop(0,'#050505');
    g.addColorStop(.42, variant === 'back' ? '#1b1710' : '#15120d');
    g.addColorStop(.72,'#090909');
    g.addColorStop(1,'#030303');
    ctx.fillStyle=g; ctx.fillRect(0,0,W,H);

    const rg=ctx.createRadialGradient(W*.86,H*.12,8,W*.86,H*.12,W*.52);
    rg.addColorStop(0,'rgba(211,176,106,.24)');
    rg.addColorStop(.38,'rgba(201,164,92,.08)');
    ...
    ctx.globalAlpha=.045;
    ctx.fillStyle='#fff';
    for(let x=24;x<W;x+=22) for(let y=24;y<H;y+=22) ctx.fillRect(x,y,1,1);
    ctx.restore();

    ctx.strokeStyle='rgba(211,176,106,.75)'; ctx.lineWidth=2;
    roundRectCanvas(ctx,2,2,W-4,H-4,24); ctx.stroke();
    ctx.strokeStyle='rgba(255,255,255,.08)'; ctx.lineWidth=1;
    roundRectCanvas(ctx,10,10,W-20,H-20,18); ctx.stroke();
  }

  async function drawIdentityFrontCanvas(ctx,W,H){
    drawLuxuryBackground(ctx,W,H,'front');
    const logo=await karsaImageForCanvas('assets/karsa-mark.jpg');
    if(logo){ roundRectCanvas(ctx,48,42,58,58,12); ctx.save();ctx.clip();ctx.drawImage(logo,48,42,58,58);ctx.restore(); }
    ctx.fillStyle='#d3b06a';ctx.font='900 46px Arial';ctx.fillText('KARSA',122,84);
    ctx.fillStyle='#bba77b';ctx.font='700 12px Arial';ctx.textAlign='right';ctx.fillText('EXECUTIVE ID',W-48,76);ctx.textAlign='left';

    const px=48,py=142,pw=174,ph=214;
    const avatar=await karsaImageForCanvas(state.user.avatarUrl||'');
    ctx.save();roundRectCanvas(ctx,px,py,pw,ph,20);ctx.clip();
    if(avatar) ctx.drawImage(avatar,px,py,pw,ph); else {ctx.fillStyle='#242424';ctx.fillRect(px,py,pw,ph);ctx.fillStyle='#d3b06a';ctx.font='800 56px Arial';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(initials(state.user.name),px+pw/2,py+ph/2);ctx.textAlign='left';ctx.textBaseline='alphabetic';}
    ctx.restore();ctx.strokeStyle='#8f6f32';ctx.lineWidth=2;roundRectCanvas(ctx,px,py,pw,ph,20);ctx.stroke();

    ctx.fillStyle='#bba77b';ctx.font='700 11px Arial';ctx.fillText('AUTHORIZED MEMBER',252,157);
    ctx.fillStyle='#f5f2eb';ctx.font='800 34px Arial';drawCanvasWrapped(ctx,state.user.name||'—',252,207,470,38);
    ctx.fillStyle='#d2bd91';ctx.font='600 17px Arial';drawCanvasWrapped(ctx,state.user.title||roleLabel(state.user.role)||'—',252,273,410,23);
    ctx.strokeStyle='#806633';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(252,304);ctx.lineTo(452,304);ctx.stroke();
    ctx.fillStyle='#f0ede6';ctx.font='500 15px Arial';ctx.fillText(karsaDisplayDivision(state.user.division),252,334);
    ctx.fillStyle='#a99d86';ctx.font='500 13px monospace';ctx.fillText(`ID ${getIdentityId()}`,252,360);

    const qr=document.querySelector('#identityQr img');
    ctx.fillStyle='#fff';roundRectCanvas(ctx,W-158,156,108,108,15);ctx.fill();
    if(qr && qr.complete && qr.naturalWidth) ctx.drawImage(qr,W-148,166,88,88);
    ctx.fillStyle='#9e8b62';ctx.font='700 9px Arial';ctx.textAlign='center';ctx.fillText('SCAN TO VERIFY',W-104,282);

    ctx.textAlign='left';ctx.fillStyle='#9e8b62';ctx.font='700 11px Arial';ctx.fillText('PT KARSA LIFESTYLE NUSANTARA',48,H-30);
    ctx.fillStyle='#d3b06a';ctx.font='800 11px Arial';ctx.textAlign='right';ctx.fillText('ACTIVE • AUTHORIZED',W-48,H-30);ctx.textAlign='left';
  }

  async function drawIdentityBackCanvas(ctx,W,H){
    drawLuxuryBackground(ctx,W,H,'back');
    const logo=await karsaImageForCanvas('assets/karsa-mark.jpg');
    if(logo){roundRectCanvas(ctx,W/2-31,45,62,62,13);ctx.save();ctx.clip();ctx.drawImage(logo,W/2-31,45,62,62);ctx.restore();}
    ctx.fillStyle='#d3b06a';ctx.font='900 40px Arial';ctx.textAlign='center';ctx.fillText('KARSA',W/2,140);
    ctx.fillStyle='#c9ad72';ctx.font='700 12px Arial';ctx.fillText('PT KARSA LIFESTYLE NUSANTARA',W/2,168);

    const items=[['COMPANY','PT Karsa Lifestyle Nusantara'],['IDENTITY','Official Internal Employee Card'],['VERIFICATION','Scan QR on front'],['STATUS','ACTIVE / AUTHORIZED']];
    const boxW=352,boxH=76,gap=14,x1=W/2-boxW-gap/2,x2=W/2+gap/2,y1=205,y2=295;
    items.forEach((it,i)=>{const x=i%2?x2:x1,y=i<2?y1:y2;ctx.fillStyle='rgba(255,255,255,.035)';ctx.strokeStyle='rgba(201,164,92,.32)';ctx.lineWidth=1;roundRectCanvas(ctx,x,y,boxW,boxH,13);ctx.fill();ctx.stroke();ctx.fillStyle='#8f8169';ctx.font='700 8px Arial';ctx.textAlign='left';ctx.fillText(it[0],x+16,y+23);ctx.fillStyle='#eee9df';ctx.font='700 13px Arial';drawCanvasWrapped(ctx,it[1],x+16,y+47,boxW-32,16);});

    ctx.strokeStyle='rgba(211,176,106,.35)';ctx.beginPath();ctx.moveTo(150,405);ctx.lineTo(W-150,405);ctx.stroke();
    ctx.fillStyle='#c9ad72';ctx.font='700 10px Arial';ctx.textAlign='center';ctx.fillText(`KARSA IDENTITY • ${getIdentityId()}`,W/2,432);
    ctx.fillStyle='#77736b';ctx.font='9px Arial';ctx.fillText('This card identifies an authorized member of PT Karsa Lifestyle Nusantara.',W/2,454);
    ctx.textAlign='left';
  }
  ...
  async function downloadIdentityCard(side){
    ...
    const W=856,H=540,canvas=document.createElement('canvas');canvas.width=W;canvas.height=H;
    const ctx=canvas.getContext('2d');if(!ctx)return;
    if(side==='back')await drawIdentityBackCanvas(ctx,W,H);else await drawIdentityFrontCanvas(ctx,W,H);
    canvas.toBlob(blob=>{if(!blob){showToast('Kartu gagal dibuat.');return;}const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`KARSA-ID-${getIdentityId()}-${side==='back'?'BACK':'FRONT'}.png`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);showToast(`Kartu ${side==='back'?'belakang':'depan'} berhasil diunduh.`)},'image/png');
  }
  ...
  async function handleKarsaVerifyQuery(){
    ...
    karsaVerifyModal();$('karsaVerifyModal').classList.remove('hidden');$('karsaVerifyBody').innerHTML=`<div class="karsa-empty-mini">Memverifikasi ${esc(id)}...</div>`;
    if(!client){$('karsaVerifyBody').innerHTML='<div class="karsa-panel"><h3>Verification unavailable</h3><p>Sistem belum terhubung.</p></div>';return;}
    const normalizedId=String(id).replace(/^KRSA-ITIT-/i,'KRSA-IT-');
    const {data,error}=await client.rpc('verify_employee',{p_employee_id:normalizedId});
    if(error||!data){$('karsaVerifyBody').innerHTML=`<div class="karsa-panel"><h3>Identity not verified</h3><p>ID <b>${esc(normalizedId)}</b> tidak ditemukan atau tidak aktif.</p></div>`;return;}
    const p=Array.isArray(data)?data[0]:data;if(!p){$('karsaVerifyBody').innerHTML=`<div class="karsa-panel"><h3>Identity not verified</h3><p>ID <b>${esc(normalizedId)}</b> tidak ditemukan atau tidak aktif.</p></div>`;return;}
    const division=karsaDisplayDivision(p.division),employeeId=karsaEmployeeId(p),photo=p.avatar_url||'';
