(() => {
  ...
  async function drawIdentityFrontCanvas(ctx, W, H) {
    ...
    // Brand header
    if (logo) {
      ctx.save(); canvasRoundRect(ctx, pad, 70, 78, 78, 20); ctx.clip(); drawCanvasCover(ctx, logo, pad, 70, 78, 78); ctx.restore();
      ctx.strokeStyle = "rgba(211,176,106,.72)"; ctx.lineWidth = 2; canvasRoundRect(ctx, pad, 70, 78, 78, 20); ctx.stroke();
    }
    ctx.fillStyle = "#d8b76f"; ctx.font = "900 62px Arial"; ctx.fillText("KARSA", 174, 128);
    ctx.fillStyle = "#a99672"; ctx.font = "700 18px Arial"; ctx.textAlign = "right"; ctx.fillText("IDENTITY • 01", W - pad, 112);
    ctx.fillStyle = "#74664f"; ctx.font = "600 13px Arial"; ctx.fillText("PT KARSA LIFESTYLE NUSANTARA", W - pad, 140);
    ctx.textAlign = "left";

    // Compact identity block
    const px = pad, py = 220, pw = 272, ph = 338;
    ctx.save(); canvasRoundRect(ctx, px, py, pw, ph, 28); ctx.clip();
    if (avatar) drawCanvasCover(ctx, avatar, px, py, pw, ph);
    else {
      ctx.fillStyle = "#171717"; ctx.fillRect(px, py, pw, ph);
      ctx.fillStyle = "#d3b06a"; ctx.font = "900 76px Arial"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(initials(state.user.name), px + pw/2, py + ph/2); ctx.textAlign = "left"; ctx.textBaseline = "alphabetic";
    }
    ctx.restore();
    ctx.strokeStyle = "rgba(211,176,106,.72)"; ctx.lineWidth = 3; canvasRoundRect(ctx, px, py, pw, ph, 28); ctx.stroke();

    const infoX = 390;
    ctx.fillStyle = "#9c875e"; ctx.font = "800 16px Arial"; ctx.fillText("AUTHORIZED MEMBER", infoX, 238);
    ctx.fillStyle = "#f5f1e9"; ctx.font = "900 46px Arial";
    drawCanvasWrapped(ctx, state.user.name || "—", infoX, 304, 690, 50);
    ctx.fillStyle = "#d5bd8b"; ctx.font = "600 25px Arial";
    drawCanvasWrapped(ctx, state.user.title || roleLabel(state.user.role) || "—", infoX, 372, 620, 30);

    ctx.strokeStyle = "rgba(201,164,92,.55)"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(infoX, 416); ctx.lineTo(infoX + 220, 416); ctx.stroke();
    ctx.fillStyle = "#f0ece3"; ctx.font = "700 22px Arial"; ctx.fillText(`DIVISION  ${normalizeKarsaDivision(state.user.division).toUpperCase()}`, infoX, 462);
    ctx.fillStyle = "#aa9c82"; ctx.font = "600 19px monospace"; ctx.fillText(`ID  ${getIdentityId()}`, infoX, 500);

    // QR, deliberately smaller and visually integrated so the card is not QR-heavy.
    const qrSize = 170, qrX = W - pad - qrSize, qrY = 250;
    ctx.fillStyle = "#fff"; canvasRoundRect(ctx, qrX, qrY, qrSize, qrSize, 20); ctx.fill();
    const qrImg = document.querySelector("#identityQr img");
    if (qrImg && qrImg.complete) { ctx.imageSmoothingEnabled = false; drawCanvasFit(ctx, qrImg, qrX + 18, qrY + 18, qrSize - 36, qrSize - 36); ctx.imageSmoothingEnabled = true; }
    ctx.fillStyle = "#9c875e"; ctx.font = "800 13px Arial"; ctx.fillText("SCAN TO VERIFY", qrX, qrY + qrSize + 27);

    // Footer kept close to the content to avoid the previous large empty area.
    ctx.fillStyle = "#776a52"; ctx.font = "700 14px Arial"; ctx.fillText("OFFICIAL EMPLOYEE IDENTITY", pad, H - 74);
    ctx.fillStyle = "#d3b06a"; ctx.font = "900 14px Arial"; ctx.textAlign = "right"; ctx.fillText("ACTIVE", W - pad, H - 74); ctx.textAlign = "left";
    ctx.fillStyle = "rgba(211,176,106,.45)"; ctx.fillRect(pad, H - 55, W - pad * 2, 1);
  }
  ...
  async function drawIdentityBackCanvas(ctx, W, H) {
    ...
    ctx.fillStyle = "#d8b76f"; ctx.font = "900 52px Arial"; ctx.fillText("KARSA", 164, 120);
    ctx.fillStyle = "#9d8961"; ctx.font = "700 16px Arial"; ctx.fillText("OFFICIAL EMPLOYEE IDENTITY", 164, 148);

    ctx.fillStyle = "#f1ede5"; ctx.font = "800 27px Arial"; ctx.fillText("PT Karsa Lifestyle Nusantara", pad, 238);
    ctx.fillStyle = "#9f927d"; ctx.font = "600 16px Arial"; ctx.fillText("KARSA INTERNAL IDENTITY SYSTEM", pad, 269);
    ...
    items.forEach((it, i) => {
      ...
      ctx.fillStyle = "#8f8068"; ctx.font = "800 12px Arial"; ctx.fillText(it[0], x + 24, y + 30);
      ctx.fillStyle = "#eee9df"; ctx.font = "700 20px Arial"; drawCanvasWrapped(ctx, it[1], x + 24, y + 67, boxW - 48, 23);
    });

    ctx.fillStyle = "#d3b06a"; ctx.font = "800 15px Arial"; ctx.textAlign = "center"; ctx.fillText(`KARSA IDENTITY  •  ${getIdentityId()}`, W / 2, H - 88);
    ctx.fillStyle = "#6f685e"; ctx.font = "600 12px Arial"; ctx.fillText("Valid for authorized internal use of PT Karsa Lifestyle Nusantara.", W / 2, H - 62);
    ctx.textAlign = "left";
  }
