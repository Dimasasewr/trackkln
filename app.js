(() => {
  ...
  async function drawIdentityBackCanvas(ctx, W, H) {
    ...
    if (logo) {
      ctx.save();
      canvasRoundRect(ctx, pad, 62, 84, 84, 20); ctx.clip();
      drawCanvasCover(ctx, logo, pad, 62, 84, 84);
      ctx.restore();
      ctx.strokeStyle = "rgba(211,176,106,.78)"; ctx.lineWidth = 2.5;
      canvasRoundRect(ctx, pad, 62, 84, 84, 20); ctx.stroke();
    }
    ctx.fillStyle = "#d8b76f"; ctx.font = "900 66px Arial"; ctx.fillText("KARSA", 180, 126);
    ctx.fillStyle = "#b19a70"; ctx.font = "800 31px Arial"; ctx.fillText("OFFICIAL EMPLOYEE IDENTITY", 180, 162);

    ctx.fillStyle = "#f1ede5"; ctx.font = "900 43px Arial"; ctx.fillText("PT Karsa Lifestyle Nusantara", pad, 238);
    ctx.fillStyle = "#b0a28b"; ctx.font = "700 28px Arial"; ctx.fillText("KARSA INTERNAL IDENTITY SYSTEM", pad, 276);

    // Subtle oversized KARSA watermark fills the lower negative space without competing
    // with the company information, QR, or footer. It mirrors the premium branding
    // treatment of the front card while remaining intentionally understated.
    ctx.save();
    ctx.globalAlpha = 0.075;
    ctx.fillStyle = "#d8b76f";
    ctx.font = "900 190px Arial";
    ctx.textAlign = "center";
    ctx.fillText("KARSA", W * 0.49, 790);
    ctx.globalAlpha = 0.045;
    ctx.font = "900 194px Arial";
    ctx.strokeStyle = "#d8b76f";
    ctx.lineWidth = 2;
    ctx.strokeText("KARSA", W * 0.49, 790);
    ctx.restore();
    ctx.textAlign = "left";

    // Compact company-information panel on the left; the right side is reserved for a larger company QR.
    const boxW = 455, boxH = 112, gapX = 22, gapY = 22;
    const x1 = pad, x2 = pad + boxW + gapX, y1 = 322, y2 = y1 + boxH + gapY;
    const items = [
    ...
    items.forEach((it, i) => {
      ...
      ctx.strokeStyle = "rgba(201,164,92,.30)";
      ctx.lineWidth = 2;
      canvasRoundRect(ctx, x, y, boxW, boxH, 20); ctx.fill(); ctx.stroke();
      ctx.fillStyle = "#b09b73"; ctx.font = "900 27px Arial"; ctx.fillText(it[0], x + 24, y + 37);
      ctx.fillStyle = "#f0ece4"; ctx.font = "800 31px Arial";
      drawCanvasWrapped(ctx, it[1], x + 24, y + 79, boxW - 48, 34);
    });

    // Company information QR. It encodes company identity directly, so it remains useful
    // even when the card is viewed offline. No unverified website/address is invented here.
    const companyQrText = [
    ...
    const qrSize = 290, qrX = W - pad - qrSize, qrY = 318;
    ctx.fillStyle = "#fff";
    canvasRoundRect(ctx, qrX - 12, qrY - 12, qrSize + 24, qrSize + 24, 28); ctx.fill();
    if (companyQr) {
      ctx.imageSmoothingEnabled = false;
      drawCanvasFit(ctx, companyQr, qrX, qrY, qrSize, qrSize);
      ctx.imageSmoothingEnabled = true;
    }
    ctx.fillStyle = "#d3b06a"; ctx.font = "900 29px Arial";
    ctx.textAlign = "center"; ctx.fillText("COMPANY INFORMATION", qrX + qrSize / 2, qrY + qrSize + 48);
    ctx.fillStyle = "#a99776"; ctx.font = "700 22px Arial";
    ctx.fillText("SCAN FOR COMPANY INFO", qrX + qrSize / 2, qrY + qrSize + 80);

    ctx.fillStyle = "#d3b06a"; ctx.font = "900 30px Arial";
    ctx.textAlign = "center"; ctx.fillText(`KARSA IDENTITY  •  ${getIdentityId()}`, W / 2, H - 92);
    ctx.fillStyle = "#8f877a"; ctx.font = "700 23px Arial";
    ctx.fillText("Valid for authorized internal use of PT Karsa Lifestyle Nusantara.", W / 2, H - 58);
    ctx.textAlign = "left";
  }
