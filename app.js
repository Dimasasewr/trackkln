(() => {
  ...
  function ensureProfileUI() {
    ...
    style.textContent = `
      ...
      .karsa-profile-modal { position:fixed; inset:0; z-index:9999; display:flex; align-items:center; justify-content:center; padding:16px; background:rgba(5,5,5,.70); backdrop-filter:blur(16px); }
      .karsa-profile-modal.hidden { display:none; }
      .karsa-profile-card { width:min(500px,100%); max-height:92vh; overflow:auto; background:#f8f7f3; color:#111; border:1px solid rgba(201,164,92,.28); border-radius:28px; padding:24px; box-shadow:0 35px 100px rgba(0,0,0,.38); animation:karsaProfileIn .26s ease both; }
      @keyframes karsaProfileIn { from{opacity:0;transform:translateY(16px) scale(.975)} to{opacity:1;transform:none} }
      .karsa-profile-head { display:flex; justify-content:space-between; align-items:flex-start; gap:16px; margin-bottom:18px; }
      .karsa-profile-head small { display:block; color:#9a7a3b; letter-spacing:.20em; font-size:9px; font-weight:900; margin-bottom:6px; }
      .karsa-profile-head h2 { margin:0; font-family:Georgia,serif; font-size:28px; letter-spacing:-.02em; }
      .karsa-profile-head p { margin:5px 0 0; color:#88847b; font-size:11px; }
      .karsa-profile-close { border:1px solid #e5e0d7; background:#fff; width:38px; height:38px; border-radius:50%; cursor:pointer; font-size:20px; color:#333; flex:0 0 auto; }
      .karsa-profile-hero { display:flex; align-items:center; gap:18px; padding:18px; border-radius:22px; background:linear-gradient(135deg,#101010,#1b1813); color:#fff; border:1px solid rgba(201,164,92,.35); box-shadow:0 15px 35px rgba(0,0,0,.16); }
      .karsa-profile-avatar-wrap { position:relative; flex:0 0 auto; }
      .karsa-profile-avatar { width:92px; height:92px; border-radius:22px; display:flex; align-items:center; justify-content:center; background:#090909; color:#d3b06a; font-size:28px; font-weight:800; background-size:cover; background-position:center; background-repeat:no-repeat; border:1px solid rgba(201,164,92,.55); box-shadow:0 10px 25px rgba(0,0,0,.25); overflow:hidden; }
      .karsa-profile-status { position:absolute; left:8px; bottom:-7px; padding:4px 7px; border-radius:999px; background:#fff; color:#6d654f; border:1px solid #e7decb; font-size:7px; font-weight:900; letter-spacing:.12em; box-shadow:0 4px 12px rgba(0,0,0,.12); }
      .karsa-profile-status i { display:inline-block; width:5px; height:5px; border-radius:50%; background:#5e9a65; margin-right:4px; }
      .karsa-profile-hero-copy { min-width:0; display:flex; flex-direction:column; gap:4px; }
      .karsa-profile-hero-copy strong { font-size:19px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      .karsa-profile-hero-copy span { color:#cfc7b7; font-size:11px; }
      .karsa-profile-actions { display:flex; justify-content:flex-start; gap:8px; margin:16px 0; flex-wrap:wrap; }
      .karsa-profile-btn { border:1px solid #ded9d0; background:#fff; color:#222; padding:10px 13px; border-radius:12px; cursor:pointer; font-weight:800; font-size:11px; }
      .karsa-profile-btn.gold { background:#c9a45c; border-color:#c9a45c; color:#fff; }
      .karsa-profile-btn.danger { color:#a64e4e; }
      .karsa-profile-btn.identity { background:#111; color:#d3b06a; border-color:#3f3625; }
      .karsa-profile-section { margin-top:14px; }
      .karsa-profile-section-head { display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; }
      .karsa-profile-section-head span { font-size:9px; font-weight:900; letter-spacing:.16em; color:#8f8060; }
      .karsa-profile-section-head small { font-size:9px; color:#aaa59c; }
      .karsa-profile-field { margin-top:0; }
      .karsa-profile-field label { display:block; font-size:9px; font-weight:900; letter-spacing:.10em; margin-bottom:6px; color:#77736b; }
      .karsa-profile-field input { width:100%; box-sizing:border-box; border:1px solid #e0dcd4; background:#fff; border-radius:13px; padding:12px 13px; font:inherit; outline:none; }
      .karsa-profile-field input:focus { border-color:#c9a45c; box-shadow:0 0 0 3px rgba(201,164,92,.12); }
      .karsa-profile-meta { padding:5px 14px; background:#fff; border:1px solid #e8e4dc; border-radius:16px; }
      .karsa-profile-meta div { display:flex; justify-content:space-between; gap:16px; padding:10px 0; border-bottom:1px solid #eeeae3; font-size:11px; }
      .karsa-profile-meta div:last-child { border-bottom:0; }
      .karsa-profile-meta span:first-child { color:#8d8981; }
      .karsa-profile-meta strong { text-align:right; max-width:65%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
      .karsa-profile-footer { display:flex; justify-content:flex-end; gap:8px; margin-top:18px; }
      .karsa-profile-note { font-size:9px; color:#969189; line-height:1.5; margin-top:12px; }
      .karsa-profile-logout { margin-top:18px; padding-top:16px; border-top:1px solid #e5e0d7; display:flex; align-items:center; justify-content:space-between; gap:12px; }
      .karsa-profile-logout div { display:flex; flex-direction:column; gap:3px; }
      .karsa-profile-logout strong { font-size:11px; }
      .karsa-profile-logout span { font-size:9px; color:#99948b; }
      .karsa-profile-logout button { border:1px solid #d9b7b7; background:#fff7f7; color:#a34545; border-radius:11px; padding:9px 13px; font-weight:900; cursor:pointer; font-size:10px; }
      @media(max-width:520px){ .karsa-profile-card{padding:18px;border-radius:23px}.karsa-profile-hero{padding:15px}.karsa-profile-avatar{width:78px;height:78px;border-radius:19px}.karsa-profile-hero-copy strong{font-size:16px}.karsa-profile-footer{flex-direction:column-reverse}.karsa-profile-footer .karsa-profile-btn{width:100%} }
    ...
    modal.innerHTML = `
      <div class="karsa-profile-card" role="dialog" aria-modal="true" aria-label="Profil pengguna">
        <div class="karsa-profile-head">
          <div><small>KARSA ACCOUNT</small><h2>Profil Saya</h2><p>Kelola identitas dan akses akun Anda.</p></div>
          <button class="karsa-profile-close" type="button" data-profile-close aria-label="Tutup">×</button>
        </div>
        <div class="karsa-profile-hero">
          <div class="karsa-profile-avatar-wrap">
            <div id="profileAvatarPreview" class="karsa-profile-avatar">K</div>
            <span class="karsa-profile-status"><i></i> ACTIVE</span>
          </div>
          <div class="karsa-profile-hero-copy">
            <strong id="profileHeroName">—</strong>
            <span id="profileHeroTitle">—</span>
            <span id="profileHeroDivision">—</span>
          </div>
        </div>
        <div class="karsa-profile-actions">
          <label class="karsa-profile-btn gold" style="cursor:pointer"><span>＋</span> Ubah Foto<input id="profileAvatarInput" type="file" accept="image/*" hidden></label>
        ...
        <section class="karsa-profile-section">
          <div class="karsa-profile-section-head"><span>DATA PERSONAL</span><small>Dapat diperbarui</small></div>
          <div class="karsa-profile-field"><label for="profileNameInput">NAMA LENGKAP</label><input id="profileNameInput" type="text" autocomplete="name"></div>
        </section>
        <section class="karsa-profile-section">
          <div class="karsa-profile-section-head"><span>INFORMASI PEKERJAAN</span><small>Read only</small></div>
          <div class="karsa-profile-meta">
            ...
            <div><span>Employee ID</span><strong id="profileEmployeeIdText">—</strong></div>
          </div>
        </section>
        <div class="karsa-profile-note">Foto akan dipotong menjadi format persegi agar konsisten di seluruh KARSA Workspace.</div>
        ...
        <div class="karsa-profile-logout">
          <div><strong>Keluar dari akun</strong><span>Sesi perangkat ini akan diakhiri.</span></div>
          <button id="profileLogoutBtn" type="button">Logout</button>
        </div>
      </div>`;
    ...
    $("profileLogoutBtn").addEventListener("click", async () => {
      if (!confirm("Keluar dari akun KARSA di perangkat ini?")) return;
      closeProfile();
      await logout();
    });
  }
  ...
  function openProfile() {
    ...
    $("profileEmployeeIdText").textContent = getIdentityId();
    $("profileHeroName").textContent = state.user.name || "—";
    $("profileHeroTitle").textContent = state.user.title || roleLabel(state.user.role);
    $("profileHeroDivision").textContent = state.user.division || "—";
    renderProfileAvatar();
  ...
  function renderProfileAvatar() {
    ...
    const fallback = state.user.initials || initials(state.user.name);
    el.textContent = fallback;
    el.style.backgroundImage = "";
    const url = state.user.avatarUrl || "";
    if (!url) return;
    const test = new Image();
    test.onload = () => { if (el.isConnected) { el.textContent = ""; el.style.backgroundImage = `url("${url}")`; } };
    test.onerror = () => { if (el.isConnected) { el.textContent = fallback; el.style.backgroundImage = ""; } };
    test.src = url;
  }
  ...
  function bind() {
    ...
    // Logout hanya tersedia dari Profil Saya, bukan dari hamburger/sidebar.
    if ($("logoutBtn")) {
  ...
  function karsaExtStyles() {
    ...
    style.textContent = `
      .karsa-person-avatar{position:relative;overflow:hidden;background:#111;display:flex;align-items:center;justify-content:center}
      .karsa-person-photo{width:100%;height:100%;display:block;object-fit:cover}
      .karsa-person-fallback{position:absolute;inset:0;align-items:center;justify-content:center;color:#fff;font-weight:800}
      .karsa-person-copy{min-width:0}
      .karsa-person-copy strong,.karsa-person-copy small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      ...
      /* FINAL INTERACTION + READABILITY FIX */
      .sidebar-backdrop{position:fixed;inset:0;display:none;background:rgba(0,0,0,.16);z-index:9990;cursor:pointer;pointer-events:none;}
      .sidebar-backdrop.show{display:block;pointer-events:auto;}
      body.sidebar-open{overflow:hidden;}
      #sidebar{z-index:9991;}
      #menuBtn{position:relative;z-index:9992;}
      ...
      #jobModal label,#jobModal .update-info,#jobModal .update-info strong,#jobModal .update-info small{color:#171717!important;}
      #jobModal .update-info{background:#faf9f6!important;border:1px solid #e6e1d7!important;border-radius:14px!important;padding:13px!important;}
      #jobModal .update-info small{color:#6f6b64!important;}
      #jobModal input,#jobModal select,#jobModal textarea{color:#171717!important;background:#fff!important;border:1px solid #d8d3ca!important;caret-color:#171717!important;}
      #jobModal input::placeholder,#jobModal textarea::placeholder{color:#8a867e!important;opacity:1!important;}
      #jobModal input:disabled,#jobModal select:disabled,#jobModal textarea:disabled{color:#555!important;background:#f3f1ed!important;opacity:1!important;-webkit-text-fill-color:#555!important;}
      #jobModal .form-actions{display:flex!important;gap:10px!important;align-items:center!important;justify-content:flex-end!important;}
      #jobModal .btn-ghost{color:#292929!important;background:#fff!important;border:1px solid #d8d3ca!important;}
      #jobModal .btn-gold{color:#fff!important;background:#b28b3f!important;border-color:#b28b3f!important;}
      #jobModal .karsa-job-extra{color:#171717!important;background:#fff!important;}
      ...
      #jobModal .karsa-comment-form input::placeholder{color:#8a867e!important;}
      #jobModal .karsa-mini-btn{color:#252525!important;background:#fff!important;}
      #jobModal .karsa-mini-btn.gold{color:#fff!important;background:#b28b3f!important;}
      #jobModal .karsa-file input[type=file]{color:#333!important;background:#fff!important;}
      #jobModal .karsa-file{background:#fff!important;color:#171717!important;}

      /* Evaluasi Mingguan */
      #view-weekly,#view-weekly *{text-shadow:none!important;}
      #view-weekly .follow-item,#view-weekly .week-card,#view-weekly .weekly-card,#view-weekly .kpi-card{color:#171717!important;}
      #view-weekly .follow-item strong,#view-weekly h1,#view-weekly h2,#view-weekly h3,#view-weekly h4,#view-weekly b{color:#171717!important;}
      #view-weekly .follow-item small,#view-weekly p,#view-weekly small{color:#66635d!important;}
      #view-weekly .empty{color:#555!important;}

      @media(max-width:700px){
        #jobModal .form-actions{position:sticky!important;bottom:0!important;background:#fff!important;padding:12px 0!important;z-index:4!important;}
        #jobModal .update-grid{grid-template-columns:1fr!important;}
        #jobModal .update-grid .wide{grid-column:1/-1!important;}
        #jobModal .karsa-comment-form{flex-direction:column!important;}
        #jobModal .karsa-comment-form .karsa-mini-btn{width:100%!important;}
      }
    `;
    document.head.appendChild(style);
  }
  ...
  function karsaAddNav() {
    ...
    wrap.addEventListener("click", e => {
      ...
      e.preventDefault();
      e.stopPropagation();
      openKarsaExtension(b.dataset.karsaExt);
    });
  }
  ...
  function closeKarsaExtension(){
    $("karsaExtOverlay")?.classList.add("hidden");
  }
  ...
  function renderKarsaCalendar(){
    ...
    $("karsaExtBody").querySelectorAll("[data-open-ext-job]").forEach(b=>b.onclick=()=>{closeKarsaExtension();closeSidebar();openJob(b.dataset.openExtJob)});
  }
  ...
  async function renderKarsaDirectory(){
    ...
    const {data,error}=await client.from("profiles").select("id,name,division,role,title,avatar_url,employee_id").order("name");
    if(error){renderPeople([]);showToast(error.message);return;}
    window.__karsaPeople=data||[];
    renderPeople(window.__karsaPeople);
    $("kPeopleSearch").oninput=e=>{const q=e.target.value.toLowerCase();renderPeople(window.__karsaPeople.filter(p=>`${p.name} ${p.division} ${p.title}`.toLowerCase().includes(q)))};
  }
  ...
  function renderPeople(people){
    ...
    grid.innerHTML=people.length?people.map(p=>{
      ...
      const initial=esc(initials(p.name));
      const image=p.avatar_url?`<img class="karsa-person-photo" data-avatar src="${esc(p.avatar_url)}" alt="" loading="lazy"><span class="karsa-person-fallback" style="display:none">${initial}</span>`:`<span class="karsa-person-fallback">${initial}</span>`;
      return `<article class="karsa-person"><div class="karsa-person-avatar">${image}</div><div class="karsa-person-copy"><strong>${esc(p.name||"—")}</strong><small>${esc(p.title||roleLabel(p.role)||"—")}</small><small>${esc(p.division||"—")}</small><small class="karsa-person-id">${esc(id)}</small></div></article>`;
    }).join(""):`<div class="karsa-empty-mini">Tidak ada data.</div>`;
    grid.querySelectorAll("img[data-avatar]").forEach(img=>{
      img.addEventListener("error",()=>{
        img.style.display="none";
        const fallback=img.parentElement?.querySelector(".karsa-person-fallback");
